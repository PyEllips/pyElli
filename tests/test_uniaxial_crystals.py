...
class TestUniaxial:
    ...
    Li = bm.Solver4x4.transition_matrix_iso_halfspace(np.array([Kx]), epsilon, inv=True)
    ...
    def test_rotated_tensor(self):
        ...
        np.testing.assert_array_almost_equal(self.filmMaterial.getTensor(self.lbda), tensor, decimal=4)
    ...
    def test_lt_matrix(self):
        n_x = 3.898 + 1j * 0.016
        cos_phi = 0.971 + 1j * 2.4e-4
        lt = np.array([[[0,         0,          cos_phi, -cos_phi],
                       [1,          1,           0,   0],
    ...
    def test_transition_matrix_with_halfspace(self):
        tmatrix = np.array([[[-1.946 - 1j*3.588, 1.668 - 1j*1.548, 0.273 * 1j * 0.03, 0.63 - 1j*0.148],
                            [1.614 + 1j*1.679, -1.989 + 1j*3.435, -0.301 - 1j*0.064, -0.861 - 1j*0.101],
                            [0.065 - 1j*0.086, 0.039 + 1j*0.097, -0.71 -1j*3.486, -0.003 - 1j*1.266],
                            [0.167 + 1j*0.403, -0.593 - 1j*0.386, -0.369 + 1j*1.199, -1.66 + 1j *3.094]]])

        np.testing.assert_array_almost_equal(self.Li @ self.Tp @ self.Lt, tmatrix, decimal=3)

    def test_jones_calculation(self):
        jones_matrix = np.array([[[-0.311 - 0.161j, -0.107 - 0.002j],
                                  [0.042 - 0.036j, -0.551 + 0.151j]]])

        np.testing.assert_array_almost_equal(self.data.jones_matrix_r, jones_matrix, decimal=3)

    def test_psi_calculation(self):
        psi = np.array([[[31.46, 10.57],
                         [5.54, 45.]]])

        np.testing.assert_array_almost_equal(self.data.psiMat, psi, decimal=2)

    def test_delta_calculation(self):
        delta = np.array([[[-42.67, -16.35],
                           [-154.93, 0.]]])

        np.testing.assert_array_almost_equal(self.data.deltaMat, delta, decimal=2)
